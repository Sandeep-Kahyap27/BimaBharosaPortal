...
public class Test_11_Ack_Status_PolicyDetailsByInsurerPage extends MainClass {
    List <String> ls;
    @Test(priority = 41)
    public void Verify_Policy_no_field_special_characters_functionality_Negative() throws InterruptedException, IOException {
        ls=register();
        ...
        Thread.sleep(1000);
        //Advance search with ack status
    ...
    public void Verify_Policy_no_field_is_blank_functionality_Negative() throws InterruptedException {
        ...
        if(vcd.verify_policy_no_blank_field_error_message()){
            test.log(Status.PASS, "policy number field is blank");
            logger.info("Test Pass: policy number field is blank");
        } else{
            test.log(Status.FAIL, "policy number field is not blank");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("Test Fail: policy number field is not blank");
        }
    }
    ...
    @Test(priority = 45)
    public void Verify_Policy_number_field_functionality_Positive() throws IOException, InterruptedException {
