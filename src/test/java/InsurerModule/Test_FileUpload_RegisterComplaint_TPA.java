...
public class Test_FileUpload_RegisterComplaint_TPA extends MainClass {
    @BeforeClass
    ...
    //Author : Abhishek
    @Test
    public void test_fileUpload_regComplaint_TPA() throws IOException {
        test = extent.createTest("Positive : Verify Complaint registered against TPA through file upload");
        ...
        e.setComplaintAgainstTypeId(ExcelFileConstants.NEW_NONLIFE, ComplaintAgainstTypeConstants.TPA, 4);
        e.setComplaintDetails(ExcelFileConstants.NEW_NONLIFE, "Register complaint against TPA through file upload", 4);

        //Set intermediary name and license number
        e.setIntermediaryName(ExcelFileConstants.NEW_NONLIFE, "Park Mediclaim TPA Private Ltd.", 4);
        e.setIntermediaryLicenseNumber(ExcelFileConstants.NEW_NONLIFE, "25", 4);
        ...
        if (up.verifySuccessMsg()) {
            test.log(Status.PASS, "Complaint registered against TPA through file upload");
            logger.info("Test Passed : Complaint registered against TPA through file upload");
        } else {
