...
public class Test_FileUpload_RegisterComplaint_Against_Surveyor extends MainClass {
    ...
    @Test
    public void test_fileUpload_regComplaint_Surveyor() throws IOException {
        test = extent.createTest("Positive : Verify Complaint registered against Surveyor through file upload");
        ...
        e.setComplaintAgainstTypeId(ExcelFileConstants.NEW_NONLIFE, ComplaintAgainstTypeConstants.SURVEYOR, 4);
        e.setComplaintDetails(ExcelFileConstants.NEW_NONLIFE, "Register complaint against SURVEYOR through file upload", 4);

        e.setIntermediaryName(ExcelFileConstants.NEW_NONLIFE,"M/S THE INDIAN SURVEYORS PVT LTD",4);
        e.setIntermediaryLicenseNumber(ExcelFileConstants.NEW_NONLIFE,"IRDA/CORP/SLA-0334",4);
        ...
        if (up.verifySuccessMsg()) {
            test.log(Status.PASS, "Complaint registered Against Surveyor through file upload");
            logger.info("Test Passed : Complaint registered Against Surveyor through file upload");
        } else {
        ...
        e.setComplaintAgainstTypeId(ExcelFileConstants.NEW_NONLIFE, ComplaintAgainstTypeConstants.INSURER, 4);
        e.setComplaintDetails(ExcelFileConstants.NEW_NONLIFE, "Register complaint through file upload", 4);

        e.setIntermediaryName(ExcelFileConstants.NEW_NONLIFE,"NIL",4);
        e.setIntermediaryLicenseNumber(ExcelFileConstants.NEW_NONLIFE,"NIL",4);
