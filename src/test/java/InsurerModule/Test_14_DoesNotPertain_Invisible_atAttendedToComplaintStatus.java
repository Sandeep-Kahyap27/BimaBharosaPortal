...
public class Test_14_DoesNotPertain_Invisible_atAttendedToComplaintStatus extends MainClass {
    List<String> lis;
    @Test(priority = 55)
    public void Verify_DoesNotPertain_IsDisable_AttendedToComplaintStatus_Positive() throws InterruptedException, IOException {
        test = extent.createTest("Positive: Verify DoesNotPertain IsDisable at AttendedTo Complaint Status.");
        lis = register();
        ...
        asp.enterComplaintNumber(lis.get(10));
        asp.clickSearchBtn();
        ViewEditComplaintDetailsPage vcdAtAttended = new ViewEditComplaintDetailsPage(driver);
        vcdAtAttended.click_View_Edit_Complaints_Checkbox();
        crg.scrollToComplaint();
        if(!vcdAtAttended.IsDisable_Does_Not_Pertain_Button()){
            test.log(Status.PASS, "Validation of Does not Pertain Checkbox is Disable at attended_to complaint status. having complaint no -"+lis.get(10));
            logger.info("Test Pass:Validation of Does not Pertain Checkbox is Disable at attended_to complaint status. having complaint no -"+lis.get(10));
        } else {
            test.log(Status.FAIL, "Does not Pertain Checkbox is not Disable at attended_to complaint status. having complaint no -"+lis.get(10));
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("Test Fail:Does not Pertain Checkbox is not Disable at attended_to complaint status. having complaint no -"+lis.get(10));
        }
    }
