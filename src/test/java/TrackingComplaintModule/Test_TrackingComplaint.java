...
public class Test_TrackingComplaint extends MainClass {
    ...
    public void test_trckng_mobno_splcharaph_chk_negative() throws InterruptedException {
        test = extent.createTest("Verify user is not able to enter special characters");
        ...
        if (t.hasSpecialCharacters(s)) {
            ...
            logger.error("Mobile number field accepting alphabet and special characters");
        } else {
            test.log(Status.PASS, "Mobile number field doesn't accept alphabet and special characters");
            logger.info("Mobile number field doesn't accept alphabet and special characters");
        }
    }

    @Test(priority = 2)
    public void test_trckng_unregistered_mobno_chk_negative() throws InterruptedException {
        test = extent.createTest("Verify user is not able to track complaint with unregistered mobile number");
        HomePage h = new HomePage(driver);
        h.clearField_mob();
        ...
        if (t.OTP_Field_visible()) {
            ...
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("With unregistered mobile number able to track complaint");
        } else {
            test.log(Status.PASS, "With unregistered mobile number not able to track complaint");
            logger.info("With unregistered mobile number not able to track complaint");
        }
    }
    ...
    public void test_trckng_registered_mobno_chk(String name, String mob, String pinCode, String compAgainst, String insCompany, String policyType, String compType, String compDescType, String policyNumber, String compDesc) throws InterruptedException {
        test = extent.createTest("Verify user is able to track complaint with registered mobile number");
        HomePage h = new HomePage(driver);
        h.clearField_mob();
        ...
        if (t.OTP_Field_visible()) {
            ...
            logger.info("With registered mobile number able to track complaint");
        } else {
            test.log(Status.FAIL, "With registered mobile number not able to track complaint");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("With registered mobile number not able to track complaint");
        }
    }

    @Test(priority = 4)
    public void test_trckng_otp_pass_chk() throws InterruptedException {
        test = extent.createTest("Verify user is able to check valid otp pass");
        HomePage h = new HomePage(driver);
        h.otp_sendkeys();
        ...
        if (h.otp_pass_chk()) {
            ...
            logger.info("After entering one digit of OTP next digit OTP field is appearing");
        } else {
            test.log(Status.FAIL, "After entering one digit of OTP next digit OTP field is not appearing");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After entering one digit of OTP next digit OTP field is not appearing");
        }
    }

    @Test(priority = 5)
    public void test_trckng_invalid_otp_chk_negative() throws InterruptedException {
        test = extent.createTest("Verify user is able to check invalid otp pass");
        HomePage h = new HomePage(driver);
        ...
        if (t.registed_complaint_visibility()) {
            ...
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After entering invalid OTP next page is appearing");
        } else {
            test.log(Status.PASS, "After entering invalid OTP next page is not appearing");
            logger.info("After entering invalid OTP next page is not appearing");
        }
    ...
    public void test_trckng_valid_Otp_VisRsndBtn() throws InterruptedException {
        test = extent.createTest("Verify user is able to check resend button visibility for valid otp");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        Thread.sleep(60000);

        if (t.resend_button_visibility()) {
            test.log(Status.PASS, "After waiting for 60 seconds resend button is visible");
            logger.info("After waiting for 60 seconds resend button is visible");
        } else {
            test.log(Status.FAIL, "After waiting for 60 seconds resend button is not visible");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After waiting for 60 seconds resend button is not visible");
        }
    }

    @Test(priority = 7)
    public void test_trckng_RsendBtn_AgainVisible() throws InterruptedException {
        test = extent.createTest("Verify user is able to check resend button visible again after passing otp");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        t.ResendBtnClk();
        Thread.sleep(60000);
        Thread.sleep(1000);
        ...
        if (t.resend_button_visibility()) {
            ...
            logger.info("After clicking resend bttton and waiting for 60 seconds resend button again visible");
        } else {
            test.log(Status.FAIL, "After completion of 60 seconds resend button is not visible");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After completion of 60 seconds resend button is not visible");
        }
    }

    @Test(priority = 8)
    public void test_trckng_splchar_otp_chk_negative() throws InterruptedException {
        test = extent.createTest("Verify user is able to enter special characters in otp field");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        HomePage h = new HomePage(driver);
        t.ResendBtnClk();
        ...
        if (t.registed_complaint_visibility()) {
            ...
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After entering special character as OTP next page is appearing");
        } else {
            test.log(Status.PASS, "After entering special character as OTP next page is not appearing");
            logger.info("After entering special character as OTP next page is not appearing");
        }
    }
    ...
    public void test_trckng_Arrow_Visible(String name, String mob, String pinCode, String compAgainst, String insCompany, String policyType, String compType, String compDescType, String policyNumber, String compDesc) throws InterruptedException {
        test = extent.createTest("Verify user is able to check visibility of arrow button for passing otp");
        HomePage h = new HomePage(driver);
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        h.enterOTP(mob);
        ...
        if (h.OTP_submit_arrow_visible()) {
            ...
            logger.info("after entering OTP arrow field is visible");
        } else {
            test.log(Status.FAIL, "after entering OTP arrow field is not visible");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("after entering OTP arrow field is not visible");
        }
    }

    @Test(priority = 10)
    public void test_trckng_valid_otp_next_page_visibility_chk() throws InterruptedException {
        test = extent.createTest("Verify user is able to check next page is visible after otp pass");
        Thread.sleep(1000);
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        HomePage h = new HomePage(driver);
        ...
        if (t.registed_complaint_visibility()) {
            ...
            logger.info("After entering valid OTP next page is appearing");
        } else {
            test.log(Status.FAIL, "After entering valid OTP next page is not appearing");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After entering valid OTP next page is not appearing");
        }
    }

    @Test(priority = 11)
    public void test_export_button_visibility() throws InterruptedException {
        test = extent.createTest("Verify user is able to check export button visibility");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        ...
        if (t.visibility_export()) {
            ...
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("Export button  is not visible");
        }
    }

    @Test(priority = 12)
    public void test_trckng_All_Field_Visible() throws InterruptedException {
        test = extent.createTest("Verify user is able to check all field visible in PHViewEditComplaint page");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);

        if (t.visibility_all_fields_of_table()) {
            test.log(Status.PASS, "all the fields visible on table.");
            logger.info("all the fields visible on table.");
        } else {
            test.log(Status.FAIL, "all the fields not visible on table");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("all the fields not visible on table");
        }
    }
    ...
    public void test_trckng_MenuBtn_Click() throws InterruptedException {
        test = extent.createTest("Verify user is able to check menu button");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        t.MenuBtnClk();
        t.complaintClk();
        ...
        if (t.reg_new_complaint_visibility() && t.view_edit_complaint_visibility()) {
            ...
            logger.info("After clicking on menu button,  complaints dropdown visible");
        } else {
            test.log(Status.FAIL, "After clicking on menu button, complaints dropdown not visible");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After clicking on menu button, complaints dropdown not visible");
        }
    }
    ...
    public void test_trckng_new_complaint_chk(String pinCode, String compAgainst, String insCompany, String policyType, String compType, String compDescType, String policyNumber, String compDesc) throws InterruptedException {
        test = extent.createTest("Verify user is able to check all complaint field");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        t.newRegisterClick();
    ...
    public void test_trckng_ShowEntries_Dropdwn() throws InterruptedException {
        test = extent.createTest("Verify user is able to check show entries dropdown button");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);

        if (t.show_entries()) {
            test.log(Status.PASS, "user able to handle show entries dropdown");
            logger.info("user able to handle show entries dropdown");
        } else {
            test.log(Status.FAIL, "user not able to handle show entries dropdown");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("user not able to handle show entries dropdown");
        }
    }
    ...
    public void test_view_edit_click() throws InterruptedException {
        test = extent.createTest("Verify user is able to check view edit check box");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        ...
        if (t.visibility_PH_update_complaint()) {
            ...
            logger.info("View edit page is visible to user");
        } else {
            test.log(Status.FAIL, "View edit page is not visible to user");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("View edit page is not visible to user");
        }
    }
    ...
    public void test_trckng_UploadedDocument() throws InterruptedException {
        test = extent.createTest("Verify user is able to check upload document field");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        ...
        if (t.visibility_uploaded_doc()) {
            ...
            logger.info("Uploaded document on Check Uploaded Attachment is visible");
        } else {
            test.log(Status.FAIL, "Uploaded document on Check Uploaded Attachment is not visible");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("Uploaded document on Check Uploaded Attachment is not visible");
        }
    ...
    public void test_trckng_SplChars_Compldetails_negative() throws InterruptedException {
        test = extent.createTest("Verify user is able to check complaint details field while passing special characters");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        ...
        if (t.visibilityOfYesBtn()) {
            ...
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("With special characters complaint description updating");
        } else {
            test.log(Status.PASS, "With special characters complaint description not updating");
            logger.info("With special characters complaint description not updating");
        }
    }
    ...
    public void test_trckng_EditInformation() throws InterruptedException {
        test = extent.createTest("Verify user is able to check edit information on PHViewEditComplaint page");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        t.clearField_complaint_desc();
        ...
        if (s1.equals(s2)) {
            ...
            logger.info("user can edit information on Complaint Details field and update the same.");
        } else {
            test.log(Status.FAIL, "user can not edit information on Complaint Details field and update the same.");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("user can not edit information on Complaint Details field and update the same.");
        }
    }
    ...
    public void test_trckng_BackBtn_click() throws InterruptedException {
        test = extent.createTest("Verify user is able to check back button function from PolicyholderUpdateComplaint page");
        TrackComplaint_Page t = new TrackComplaint_Page(driver);
        t.view_edit_button_click();
        ...
        if (t.registed_complaint_visibility()) {
            ...
            logger.info("After click on back button on PHViewEditComplaint page user able to back on previous page");
        } else {
            test.log(Status.FAIL, "After click on back button on PHViewEditComplaint page user not  able to back on previous page");
            test.fail(new Throwable(), MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot.captureScreenShot()).build());
            logger.error("After click on back button on PHViewEditComplaint page user not  able to back on previous page");
        }
    }
}
